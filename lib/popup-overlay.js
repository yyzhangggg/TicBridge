...
export function initPopupOverlay(platformId, autofillApi, { openOnLoad = false } = {}) {
  const togglePanel = () => {
  ...
  // Some creator-center SPAs retain a tab across navigations. On the Douyin
  // landing page we open once as soon as the script is available, so the UI is
  // not dependent on the toolbar action reaching a freshly-created listener.
  if (openOnLoad) queueMicrotask(togglePanel);
}
