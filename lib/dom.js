...
export function candidateList(selectors) {
...
/** Same priority-order walk as queryFirst, but returns every element matched
 *  by the first candidate selector that matches anything, instead of just one.
 *  Used by preflight to tell "found exactly one field" apart from "this selector
 *  is too loose and caught several elements" (a silent wrong-field risk if we
 *  just filled whichever one document.querySelector happened to return). */
export function findFieldCandidates(selectors) {
  for (const sel of candidateList(selectors)) {
    const matches = Array.from(document.querySelectorAll(sel));
    if (matches.length > 0) return { selector: sel, matches };
  }
  return { selector: null, matches: [] };
}

/** Best-effort check that an element is actually something a real user could
 *  click into and type in right now — not just present in the DOM. Selector
 *  matching a <input> that's disabled, display:none, zero-size, or behind an
 *  earlier unfinished step (e.g. a step-2 field before step-1 is done) is the
 *  kind of "found it but can't use it" failure a plain querySelector can't see. */
export function isInteractable(el) {
  if (!el || !el.isConnected) return false;
  if (el.disabled) return false;
  if (el.tagName !== "DIV" && el.hasAttribute("readonly")) return false;
